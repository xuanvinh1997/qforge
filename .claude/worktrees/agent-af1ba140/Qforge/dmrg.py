...
class DMRG:
    ...
    def run(
        ...
        if (self.backend == 'cpu' and _HAS_MPS_CPP
                and self._hamiltonian_cpp is not None
                and self._psi._mps is not None):
            energy = self._run_cpp(n_sweeps, convergence_tol, verbose)
        else:
            energy = self._run_python(n_sweeps, convergence_tol, verbose)

        return energy, self._psi

    def _run_cpp(self, n_sweeps: int, tol: float, verbose: bool) -> float:
        energy = 0.0
        for sweep in range(n_sweeps):
            prev = energy
            energy = _dmrg_sweep_cpp(
    ...
    def _run_python(self, n_sweeps: int, tol: float, verbose: bool) -> float:
